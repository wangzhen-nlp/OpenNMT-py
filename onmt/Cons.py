...
class ConstraintHypothesis:
    ...
    def __init__(self, token, score, coverage, constraints, payload=None, backpointer=None,
                 constraint_index=None, unfinished_constraint=False, blacks=None):
        self.token = token
        self.score = score
...
def unfinished(hyp, eos=onmt.IO.EOS_WORD):
    if hyp.token == eos:
        return False
    return True


def eos_covers_constraints(hyp, eos=set([onmt.IO.EOS_WORD])):
    """
...
class ConstrainedDecoder(object):
    ...
    def search(self, start_hyp, constraints, max_hyp_len, beam_size=10):
        ...
        for i in range(1, max_hyp_len + 1):
            ...
            for j in range(j_start, min(i, grid_height) + 1):
                ...
                # print(sorted([h['score'] for h in new_beam.hypotheses]))
                # input()
    ...
    def best_n(search_grid, eos_token, n_best=1, cut_off_eos=True, return_model_scores=False, return_alignments=False,
               length_normalization=False, prefer_eos=True):
        top_row = max(k[1] for k in search_grid.keys())
        top_height = max(k[0] for k in search_grid.keys())
        ...
        if top_row > 0:
            ...
            # output_beams = [search_grid[k] for k in search_grid.keys() if k[0] != 0]
