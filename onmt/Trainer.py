    ...
    def validate(self):
        ...
        for batch in self.valid_iter:
            ...
            gen_state = onmt.Loss.make_gen_state(
                outputs, batch, attns, (0, batch.tgt[0].size(0)))
...
class VAETrainer(object):
    ...
    def train(self, epoch, report_func=None):
        ...
        for i, batch in enumerate(self.train_iter):
            ...
            for j in range(0, target_size-1, trunc_size):
                ...
                # 2. F-prop all but generator.
                self.model.zero_grad()
                outputs, attns, dec_state, kld, hidden = \
                    self.model(src, tgt, src_lengths, tgt_lengths, dec_state)

                # 3. Compute loss in shards for memory efficiency.
                batch_stats = self.train_loss.sharded_compute_loss(
                        batch, outputs, attns, j,
                        trunc_size, self.shard_size, hidden, kld)
    ...
    def validate(self):
        ...
        for batch in self.valid_iter:
            ...
            # F-prop through the model.
            outputs, attns, _, kld, hidden = self.model(src, tgt, src_lengths, tgt_lengths)

            # Compute loss.
            gen_state = onmt.Loss.make_gen_state(
                outputs, batch, attns, (0, batch.tgt[0].size(0)), hidden=hidden)
            _, batch_stats = self.valid_loss(batch, **gen_state)
    ...
    def drop_checkpoint(self, opt, epoch, fields, valid_stats):
        ...
        real_bow_project = (real_model.bow_project.module
                            if isinstance(real_model.bow_project, nn.DataParallel)
                            else real_model.bow_project)

        model_state_dict = real_model.state_dict()
        model_state_dict = {k: v for k, v in model_state_dict.items()
                            if 'generator' not in k and 'bow_project' not in k}
        generator_state_dict = real_generator.state_dict()
        bow_project_state_dict = real_bow_project.state_dict()
        checkpoint = {
            'model': model_state_dict,
            'generator': generator_state_dict,
            'bow_project': bow_project_state_dict,
            'vocab': onmt.IO.ONMTDataset.save_vocab(fields),
            'opt': opt,
            'epoch': epoch,
            'optim': self.optim
